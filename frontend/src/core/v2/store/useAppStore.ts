...
export interface AppState {
  ...
  fetchProducts: (opts?: FetchProductsOptions) => Promise<Product[]>;
  fetchProductDetails: (slugOrId: string | number) => Promise<Product | null>;
  fetchCategories: (opts?: { skip?: number; limit?: number }) => Promise<Category[]>;
  fetchBrands: (opts?: { skip?: number; limit?: number }) => Promise<Brand[]>;
  fetchCombos: (opts?: { limit?: number }) => Promise<Combo[]>;
  ...
  modalSize: string;
  modalConcentration: string;

  isQuizOpen: boolean;
  quizStep: number;
  quizAnswers: Record<string, any>;
  quizRecommendation: Product | null;
  ...
  setUser: (user: User | null, tokens?: { accessToken: string; refreshToken: string } | null) => void;
  setAuthModal: (isOpen: boolean, mode?: 'login' | 'register' | 'profile') => void;
  setWishlist: (wishlist: string[]) => void;
  setCardSelections: (updater: Record<string, CardSelection> | ((prev: Record<string, CardSelection>) => Record<string, CardSelection>)) => void;
  setSelectedProduct: (product: Product | null) => void;
  setModalSize: (size: string) => void;
  setModalConcentration: (concentration: string) => void;
  setIsQuizOpen: (isOpen: boolean | ((prev: boolean) => boolean)) => void;
  setQuizStep: (step: number) => void;
  setQuizAnswers: (answers: Record<string, any>) => void;
  setQuizRecommendation: (recommendation: Product | null) => void;
  setIsCheckoutMode: (isCheckout: boolean | ((prev: boolean) => boolean)) => void;
  ...
  setShippingAddress: (updater: ShippingAddress | ((prev: ShippingAddress) => ShippingAddress)) => void;
  setShippingInfo: (updater: ShippingInfo | ((prev: ShippingInfo) => ShippingInfo)) => void;
  setPaymentDetails: (updater: PaymentDetails | ((prev: PaymentDetails) => PaymentDetails)) => void;
  setOrderCompleted: (completed: boolean) => void;
  setIsProcessingOrder: (isProcessing: boolean) => void;
  setToasts: (updater: Toast[] | ((prev: Toast[]) => Toast[])) => void;

  saveCart: (newCart: CartItem[]) => void;
  saveWishlist: (newWishlist: string[]) => void;
  addToast: (text: string, type?: Toast['type']) => void;
  calculateItemPrice: (basePrice: number, size: string, concentration: string) => number;
  handleAddToCart: (product: Product, size: string, concentration: string, qty?: number, explicitUnitPrice?: number | null) => void;
  handleAddComboToCart: (combo: Combo, qty?: number) => void;
  handleUpdateQty: (itemId: string, change: number) => void;
  handleRemoveFromCart: (itemId: string) => void;
  toggleWishlist: (productId: string) => void;
  startQuiz: () => void;
  handleQuizAnswer: (question: string, answer: any) => void;
  handleCheckoutSubmit: (e?: React.FormEvent) => Promise<void>;
  handleResetCheckout: () => void;
  handleOpenProductDetail: (product: Product) => void;
  applyPromoCode: (e?: React.FormEvent) => Promise<void>;
  removePromoCode: () => void;

  getFilteredProducts: () => Product[];
  getCartPricing: () => CartPricing;
}
