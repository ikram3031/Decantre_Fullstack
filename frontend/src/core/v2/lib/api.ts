...
export const getImageBaseUrl = (): string => {
  const envImgUrl =
    ...
    (import.meta as any).env?.NEXT_PUBLIC_IMAGE_BASE_URL ||
    "";
...
const clearStoredMemberTokens = (): void => {
  if (typeof window === "undefined") return;
  localStorage.removeItem("luxury_access_token");
  localStorage.removeItem("luxury_refresh_token");
};

const refreshMemberSession = async (): Promise<{
  accessToken: string | null;
  refreshToken: string | null;
}> => {
...
): Promise<Product[] & { _meta?: any; _totalRows?: number }> {
  const apiBaseUrl = getApiBaseUrl();
  ...
  try {
    ...
    const list = Array.isArray(json.data)
      ...
      : Array.isArray(json)
        ? json
        : [];
    const mapped = list.map(mapRemoteProduct) as Product[] & {
      _meta?: any;
      _totalRows?: number;
    };
 ...
 * Create Order
 */
export async function createOrder(orderPayload: OrderPayload): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
 ...
 * Check Member Email
 */
export async function checkMemberEmail(emailPayload: { email: string }): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
 ...
 * Member Login
 */
export async function loginMember(credentials: any): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
 ...
 * Resend Member OTP
 */
export async function resendMemberOtp(payload: any): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
 ...
 * Register Member
 */
export async function registerMember(memberPayload: any): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
 ...
 * Verify Member OTP
 */
export async function verifyMemberOtp(payload: any): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
 ...
 */
export async function forgotMemberPassword(payload: any): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
 ...
 */
export async function resetMemberPassword(payload: any): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
 ...
 */
export async function refreshMemberToken(payload: any): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
 ...
 */
export async function logoutMember(payload: any): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
...
export async function fetchMemberById(memberId: string | number): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
...
export async function createMember(memberPayload: any): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
  const res = await authFetch(`${apiBaseUrl}/api/v1/members`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function updateMember(memberId: string | number, updatePayload: any): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
  const res = await authFetch(`${apiBaseUrl}/api/v1/members/${memberId}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
export async function deleteMember(memberId: string | number): Promise<any> {
  const apiBaseUrl = getApiBaseUrl();
  const res = await authFetch(`${apiBaseUrl}/api/v1/members/${memberId}`, {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
