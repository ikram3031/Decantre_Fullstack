...
import { InvoiceOrder } from '../../lib/invoiceUtils';

interface OrderSuccessReceiptProps {
  successOrder: InvoiceOrder;
  onBack: () => void;
  onPrint: (order: InvoiceOrder) => void;
  onDownload: (order: InvoiceOrder) => void;
}

export const OrderSuccessReceipt: React.FC<OrderSuccessReceiptProps> = ({
  successOrder,
  onBack,
  onPrint,
  onDownload,
          ...
          <div className="border-t border-slate-100 pt-5">
            ...
            <div className="divide-y divide-slate-50">
              {successOrder.items.map((item, idx) => (
                <div key={item.productId || item.id || idx} className="py-2.5 flex justify-between items-center text-xs">
                  <div>
