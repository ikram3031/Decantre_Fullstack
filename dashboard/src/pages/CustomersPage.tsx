...
export const CustomersPage: React.FC = () => {
  ...
  const { data: customersData = { items: [], total: 0 }, isLoading } = useQuery<{ items: Customer[]; total: number }>({
    queryKey: ['customers'],
    queryFn: async () => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    } else if (selectedCustomer && custId) {
  ...
  const sortedCustomers = [...filteredCustomers].sort((a: any, b: any) => {
    ...
    if (sortBy === 'totalSpent' || sortBy === 'orders') {
      const aValue = sortBy === 'orders' ? (a.orders?.length || 0) : (a.orders?.reduce((sum: number, o: any) => sum + (o.total || 0), 0) || 0);
      const bValue = sortBy === 'orders' ? (b.orders?.length || 0) : (b.orders?.reduce((sum: number, o: any) => sum + (o.total || 0), 0) || 0);
            ...
            <table className="w-full text-left text-sm text-slate-600">
              ...
              <tbody className="divide-y divide-slate-100">
                {sortedCustomers.map((c: any) => {
                  ...
                  return (
                    <tr key={custId} className="hover:bg-slate-50/50 transition">
                          ...
                          <button
                            onClick={() => {
                              if (confirm('Are you sure you want to delete this customer?')) {
                                deleteCustomerMutation.mutate(custId);
                              }
                            }}
              ...
              <div>
                ...
                {!isAddMode && selectedCustomer && (
                  <span className="text-[10px] text-slate-400 font-mono font-bold mt-1 block">
                    Customer ID: {selectedCustomer.id || selectedCustomer._id}
                  </span>
                )}
              </div>
              <button
