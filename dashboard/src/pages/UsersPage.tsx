...
export const UsersPage: React.FC = () => {
  ...
  const { data: usersData = { items: [], total: 0 }, isLoading } = useQuery<{ items: UserProfile[]; total: number }>({
    queryKey: ['users'],
    queryFn: async () => {
  ...
  const addUserMutation = useMutation({
    mutationFn: async (newUser: Partial<UserProfile>) => {
  ...
  const updateUserMutation = useMutation({
    mutationFn: async (data: { id: string | number; updates: Partial<UserProfile> }) => {
  ...
  const handleOpenEdit = (u: UserProfile) => {
    ...
    setFormRole(u.role || 'Store_manager');
    setFormPassword('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingUser && userId) {
      updateUserMutation.mutate({ id: userId, updates: payload });
    } else {
