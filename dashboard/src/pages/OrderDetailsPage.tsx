...
export const OrderDetailsPage: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    // Calculate new total
    const newTotal = editItems.reduce((sum, item) => sum + (Number(item.price || 0) * Number(item.quantity || 1)), 0);
              ...
              {itemsToShow.map((item: any, idx: number) => (
                <div key={idx} className="p-6 flex gap-4 items-start hover:bg-slate-50/30 transition">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 font-mono mt-1">Product ID: {item.productId || item.id}</p>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-xs p-6 space-y-6">
            ...
            <div className="space-y-4 text-xs">
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-950">{order.customerName || order.customer?.name || 'Guest Customer'}</p>
                  <p className="text-slate-400 font-semibold font-mono mt-0.5">{order.customerId || order.customer?.id || 'N/A'}</p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-slate-600">
                  ...
                  <span>{order.customerEmail || order.customer?.email || 'Not provided'}</span>
                </div>
                <div className="flex items-center gap-2 text-slate-600">
                  ...
                  <div className="flex-1 min-w-0 bg-slate-50/20 hover:bg-slate-50/50 border border-slate-100/50 p-3 rounded-xl">
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <span className="text-[9px] text-slate-400 font-mono shrink-0">
                        {new Date(h.date || h.createdAt).toLocaleDateString(undefined, {
                          month: 'short',
                          day: 'numeric',
                          hour: '2-digit',
                          minute: '2-digit'
