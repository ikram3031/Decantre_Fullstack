...
export const printInvoice = (order: InvoiceOrder): void => {
  ...
  const orderDate = order.date ? new Date(order.date).toLocaleDateString() : new Date().toLocaleDateString();
      ...
      <div class="meta-info">
        <table>
          <tr>
            ...
            <td class="text-right"><strong>Date:</strong> ${orderDate}</td>
          </tr>
          <tr>
      ...
      <table class="items-table">
        ...
        <tbody>
          ${order.items.map((item) => `
            <tr>
              <td>${item.name || 'Product'}${item.size ? ` (${item.size})` : ''}</td>
...
export const downloadInvoice = (order: InvoiceOrder): void => {
  const orderDate = order.date ? new Date(order.date).toLocaleDateString() : new Date().toLocaleDateString();
  const paymentMethod = order.paymentMethod || 'Cash';
      ...
      <div class="invoice-box">
        <table cellpadding="0" cellspacing="0">
              ...
              <table>
                <tr>
                  ...
                  <td>
                    ...
                    <strong>Created:</strong> ${orderDate}<br>
          ...
          ${order.items.map((item) => `
            <tr class="item">
              <td>
                <strong>${item.name || 'Product'}${item.size ? ` (${item.size})` : ''}</strong><br>
                <span style="font-size: 11px; color: #777;">Qty: ${item.quantity || 1} @ ৳${Number(item.price || 0).toFixed(2)} each</span>
              </td>
